    ...
    <>
      ...
      <div style={{ textAlign: 'center', backgroundColor: '#007bff', minHeight: '100vh' }}>
        <div className="blogs-page" style={{ padding: '20px' }}>
          <Link to="/HealthcareProviderCommunity" className="create-blog-btn" style={{ marginBottom:'30px', textDecoration: 'none', color: '#28a745', backgroundColor: '#ffff', padding: '12px 24px', borderRadius: '6px', fontWeight: 'bold', marginTop: '30px', display: 'inline-block' }}>Go Back</Link>
          <h1  variant="h4" gutterBottom style={{ color: '#ffff', marginBottom: '20px', fontWeight: 'bold' }}>📖 My Blogs</h1>
          <Link to="/HealthcareProviderAddBlog" className="create-blog-btn" style={{ textDecoration: 'none', color: '#fff', backgroundColor: '#28a745', padding: '12px 24px', borderRadius: '6px', fontWeight: 'bold', marginBottom: '20px', display: 'inline-block' }}>Create Blog</Link>
          <div className="blog-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '20px', alignItems: 'stretch' }}>
            {blogs.map((blog, index) => (
              <div key={blog.BlogID} className="blog-card" style={{ backgroundColor: '#fff', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', borderRadius: '8px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <Link to={`/HealthcareProviderSingleBlogPage/${blog.BlogID}`} style={{ textDecoration: 'none', color: '#333', width: '100%' }}>
